...
export const generateReportCSV = (report: ExportableReportData): string => {
  ...
  if (report.recommendations && Array.isArray(report.recommendations)) {
    rows.push('Shipment Details');
    rows.push('Tracking ID,Origin ZIP,Destination ZIP,Weight,Current Service,Current Cost,Recommended Service,Recommended Cost,Savings,Savings %');
    
    report.recommendations.forEach((shipment: any) => {
      const currentCost = shipment.currentRate || 0;
      const newCost = shipment.newRate || 0;
